...
# --- Ensure published column is proper datetime ---
...
with mlflow.start_run(run_name="feature_engineering"):
    ...
    # --- Log high negative days ---
    high_neg_days = daily_sentiment[daily_sentiment["percent_negative"] >= 35]
    high_neg_path = os.path.join(PROCESSED_PATH, "high_negative_days.txt")
    with open(high_neg_path, "w") as f:
        for idx, row in high_neg_days.iterrows():
            f.write(f"{row['Date']} — {row['percent_negative']:.1f}% negative news: {row['investment_advice']}\n")
